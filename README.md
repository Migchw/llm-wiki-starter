...
`scripts/fetch_source.py` เติมรหัสจากแหล่งที่มาและเนื้อหาในชื่อ capture เพื่อแยกบทความชื่อซ้ำหรือชื่อภาษาไทยที่ย่อเป็น slug เดียวกัน หากชื่อฐาน แหล่งที่มา และ bytes ที่จะบันทึกเหมือนเดิม จะใช้ไฟล์เดิมโดยไม่เขียนซ้ำ เมื่อเนื้อหาหรือ metadata เปลี่ยน (รวมถึงวันที่ capture) จะสร้างไฟล์ใหม่ และเก็บหลักฐานเดิมไว้

เอกสารที่ดาวน์โหลดจะแปลงจากไฟล์ชั่วคราวก่อนบันทึกต้นฉบับแยกตาม capture; `raw_file` ในโน้ตจะชี้ไปยังไฟล์ต้นฉบับของ capture นั้น

ทดสอบการเก็บ raw แบบ offline โดยไม่ต้องติดตั้งไลบรารีเสริม:
